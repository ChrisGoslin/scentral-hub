...
test.describe('Archive Import', () => {
  ...
  test('renders a preview for a signed-in archive import', async ({ page }) => {
    ...
    await page.route('**/api/portability/preview', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await page.getByRole('textbox', { name: /Import source text/i }).fill('brand,name\nDior,Sauvage')
    await page.getByRole('button', { name: /Preview import/i }).evaluate((node: HTMLButtonElement) => node.click())
