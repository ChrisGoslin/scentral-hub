...
test.describe('Shelf Page', () => {
  ...
  test('shows the signed-out shelf and routes sign in to login', async ({ page }) => {
    ...
    await expect(page.getByRole('status', { name: /Your bottles are waiting/ })).toBeVisible({ timeout: 20_000 });
    await expect(page.getByRole('status', { name: /Sign in to arrange your Top 20/ })).toBeVisible();

    await page.getByRole('link', { name: 'Sign in' }).click();

    await expect(page).toHaveURL(/\/login\?next=(%2F|\/)shelf$/);
    await expect(page.getByRole('status', { name: /Come back to your shelf/ })).toBeVisible({ timeout: 10_000 });
  });
