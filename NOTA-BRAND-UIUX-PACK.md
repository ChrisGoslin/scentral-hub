...
## 10. Surface glossary — reconciled to shipped routes (verified against live code 2026-07-24)

The homepage/hero build on `brand/sensory-sanctuary` surfaced the real route
map via failing E2E specs; a follow-up pass then read the actual route files
(`app/(main)/{study,lab,cabinet,collection,shelf}/page.tsx`) and corrected one
row that the 2026-07-22 version got wrong. Shipped routes and code win over
doctrine names:

| Confirmed route | UI name | Notes |
|---|---|---|
| `/read` | **The Read** | matches doctrine, no change |
| `/cabinet` | **Cabinet** | renders the legacy `collections`-table "Living Wardrobe" (`CollectionClientWrapper`). `/collection` is a confirmed `permanentRedirect` shim to `/cabinet`, not a live duplicate |
| `/shelf` | **Shelf** | **NOT retired — this table previously said "Shelf → Cabinet"; that was wrong.** `/shelf` is a separate, live, actively-built route (`shelf_items` table, 20-slot S/A/B/C tiers) distinct from Cabinet. Two real surfaces, two real tables — see the flag below |
| `/study` | **Study** | retires "Discover" as a label; renders `DiscoverClient`, a fragrance catalogue browse/search. **No Profiler/Trails content exists here** — do not describe this surface as a "learning" surface in copy |
| `/lab` | **Lab** | retires "Layering" as a UI label; confirmed distinct formulation/chemistry workbench (`LayeringClient`), **not** the same thing as Traces — do not merge |

**Cabinet vs. Shelf — flagged, not resolved:** both routes are live and both
claim ownership of "what the user owns/wears," backed by two different tables
(`collections` vs `shelf_items`). This is a genuine product/data duplication
(see `CLAUDE.md` §5–6), not a naming cleanup — needs a human decision on
whether to merge them or make the Cabinet/Shelf distinction legible to users
(e.g. archive vs. curated top-20).
...
ScentBloom™, Scent Tarot™, nota.lab, Trace Composer, and the UI label
"Discover" (route renamed to Study). **"Shelf" is not retired** — corrects the
