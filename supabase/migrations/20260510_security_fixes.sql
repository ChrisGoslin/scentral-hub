...
BEGIN
  IF EXISTS (
    ...
    -- 2. Revoke direct execute access from anon, authenticated, and the
    --    default PUBLIC grant. handle_new_user is a trigger function — it
    --    should only be called by the trigger, never directly via the REST
    --    API. Revoking only anon/authenticated leaves it callable via the
    --    grant every role inherits from PUBLIC.
