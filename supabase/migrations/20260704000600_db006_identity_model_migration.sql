...
--
-- 20260703000500_shelf_events_table.sql and 20260703000600_temptations.sql
-- were both rewritten (2026-07-18) to match the live production shape,
-- which is already user_id-only with no anon_id column at all — this
-- migration's original unconditional "add user_id, keep anon_id, support
-- both" steps for both tables would now fail (duplicate column / anon_id
-- doesn't exist). docs/HANDOVER.md also documents DB-006 as already
-- applied to production. Steps 1/2 and the temptations/shelf_events
-- halves of steps 6/7 are guarded to no-op wherever anon_id is already
-- absent.
...
-- Step 5: Add indexes for new user_id columns (same as anon_id). Both
-- temptations and shelf_events use IF NOT EXISTS since the rewritten
...
--
-- The temptations half is wrapped in dynamic SQL guarded on anon_id still
-- existing, same reasoning as shelf_events below: a bare CREATE POLICY
-- referencing anon_id fails at execution time once temptations no longer
-- has that column. Where it's already gone, the rewritten migration
-- already created the correct final "own rows" policy.
