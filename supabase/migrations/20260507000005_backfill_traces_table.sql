...
-- this repo creates it — 20260703000700_trace_reactions_table.sql references
-- traces(id) via FK without anything creating the table first, causing that
-- migration's guard to always return early and leave trace_reactions
-- permanently absent while app/api/traces/[id]/react/route.ts and the trace
-- feeds query it at runtime. Same phantom-object pattern as handle_new_user/
-- profiles found earlier this session. Same fix already applied on PR64.
-- Guarded on profiles and fragrances (its FK targets) both existing.
