...
steps(position, step_type, content) AS (
  VALUES
    (1, 'hook', jsonb_build_object(
...
SELECT trail.id, steps.position, steps.step_type, steps.content
FROM trail CROSS JOIN steps;
