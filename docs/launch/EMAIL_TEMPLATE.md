# nota. Launch Email — Waitlist Notification

**Subject line options (A/B test):**
- A: "Your fragrance wardrobe is ready (3 questions to find it)"
- B: "nota. is live—find your £18 dupe for £150"
- C: "You're on the list. We launched."

**Send time:** T+1 (day after launch) @ 9am recipient's timezone (if available, else 9am UTC)

---

## Email Body

**From:** Christopher (hello@scentral.com) or nota. Team
**Reply-to:** help@scentral.com

---

```
Hi there,

nota. is live.

You signed up months ago. Here's what we built.

THE SANCTUARY PROFILER (15 seconds)
We ask you one question: where are you when the world gets loud?

Your answer becomes your fragrance DNA—one of three personas:
• Velvet Intellectual (moody, introspective, niche)
• Solar Minimalist (bright, decisive, clean)
• Dark Alchemist (experimental, sensual, chaotic)

THE WARDROBE (instant)
Your persona reveals 50+ fragrances curated for you.

Each one has:
✦ A price dupe (your £150 Creed → a £18 Lattafa that smells 85% the same)
✦ A mood match (ritual, season, moment)
✦ Social proof (% of nota. users who own it)

THE CATCH
No account. No login. No algorithm. No BS.

Open the app. Take 15 seconds. See your wardrobe.

TRY IT NOW
→ scentral-hub.vercel.app

OR if you're on mobile:
→ App Store: [link]
→ Google Play: [link]

---

Questions? I read every email. Reply to this or find me on Twitter @christopher.

—C
Founder, nota.
```

---

## Plain-Text Version (for email clients that strip HTML)

```
Hi there,

nota. is live. You signed up months ago. Here's what we built.

THE SANCTUARY PROFILER (15 seconds)
We ask one question: where are you when the world gets loud?

Your answer becomes your fragrance DNA—one of three personas:
Velvet Intellectual (moody, introspective, niche)
Solar Minimalist (bright, decisive, clean)
Dark Alchemist (experimental, sensual, chaotic)

THE WARDROBE (instant)
Your persona reveals 50+ fragrances curated for you.

Each one has:
- A price dupe (your £150 Creed → £18 Lattafa, 85% the same smell)
- A mood match (ritual, season, moment)
- Social proof (% of nota. users who own it)

THE CATCH
No account. No login. No algorithm. No BS.

Open the app. Take 15 seconds. See your wardrobe.

TRY IT NOW
scentral-hub.vercel.app

Questions? I read every email. Reply to this or find me on Twitter @christopher.

—C
Founder, nota.
```

---

## Segmentation Notes
- **If you have email data:** segment by signup reason (fragrance interest vs vague "early access")
- **Optional personalization:** if user mentioned a fragrance in signup, call it out: "You asked about Creed Royal Oud. We found your dupe (and 5 others)."

## Follow-up Cadence (T+3, T+7, T+14)
- **T+3:** "Here's what early users found" (social proof email with download count + persona distribution)
- **T+7:** "Don't sleep on the dupes" (feature highlight email with example dupe chains)
- **T+14:** "Your wardrobe is waiting" (last re-engagement email before marketing pause)

Use same tone: direct, personal, no fluff.
