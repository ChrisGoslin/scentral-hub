...
> - **Taupe hex — hex RESOLVED 2026-07-26; contrast claim RE-OPENED 2026-08-19.** `#766E64` is the single canonical value across `DESIGN.md`, `NOTA_MANIFESTO.md`, `NOTA-BRAND-UIUX-PACK.md`, `docs/brand/nota-imagery-briefs.md`, `docs/nota/09-brand-design-handover.md`, and shipped `app/globals.css`; the old `#B8AC9C` is fully retired. **That part stands.** The accompanying claim — "10.35:1 on ivory, clears WCAG AA for any text" — was never measured and is false. Re-measured 2026-08-19 with the WCAG 2.x relative-luminance formula: **`#766E64` on `#F7F4EE` is 4.57:1**, clearing AA normal by 0.07 with no headroom for the 2.8% grain multiply. `#756A5C` (taupe-ink) at 4.82:1 is the load-bearing text token. All eight palette tokens were recomputed in the same pass; the other seven matched their published figures exactly, which is why this one is a hand-entered error rather than a systematic one. Corrected in `DESIGN.md` §2 and `NOTA-BRAND-UIUX-PACK.md` §2.2. **Still open:** `app/globals.css` defines no `--taupe-ink` and derives `--color-text-muted` by *lightening* taupe (`#989188`, 2.85:1 on ivory) — a shipped-code gap, not a doc gap. See the flagged block in `DESIGN.md` §2.
>
