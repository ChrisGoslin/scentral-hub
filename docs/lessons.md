...
**Enforced by:** `.husky/pre-push`, positioned as the FIRST check on every branch — if the hook being run is not the committed one, no later check in the file can be trusted to have run either.
