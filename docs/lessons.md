...
**Enforced by:** advisory only — `grep -n "^### L" docs/lessons.md | sort -t L -k2 -n | uniq -d -f1` (or equivalent) would mechanically catch a repeat ID; no CI job runs this yet. Adding one is a candidate small follow-up, not done in this pass.
