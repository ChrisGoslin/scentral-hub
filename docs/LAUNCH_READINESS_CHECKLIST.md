...
## 2026-08-10 ordered launch checklist

This section supersedes the undated assumptions in the older Phase 7 checklist below. It is a web-launch checklist for nota.'s chemist-workshop experience; app-store, waitlist-volume, persona, XP, and generic beauty-launch claims below remain historical until re-verified.

| Order | Launch outcome | Evidence | Owner | Metric | Effort | Next action |
|---|---|---|---|---|---|---|
| 1 | A new visitor understands the workshop promise and has one clear entry into the experience. | `app/page.tsx`, `app/welcome/page.tsx`, `app/onboarding/page.tsx` | Christopher (message) + product engineering (route) | Homepage-to-onboarding start rate; baseline first, then improve | Small | Read the live homepage and onboarding as one journey; remove any promise the product cannot demonstrate. |
| 2 | Search engines can discover the public workshop surfaces without indexing private or thin utility routes. | **Blocked:** `app/robots.ts` and `app/sitemap.ts` do not exist as of 2026-08-10; only layout metadata and public route files are present. | Product engineering | Valid robots/sitemap; indexed public-route count; crawl errors | Small | Add and validate robots/sitemap metadata against the live public route inventory, with intentional exclusions documented. |
| 3 | The first-run path reaches a meaningful scent result without a dead end. | `app/onboarding/page.tsx`, `app/noseprint/page.tsx`, `app/(main)/discover/page.tsx`, relevant Playwright specs | Product engineering | Onboarding start-to-completion rate; median completion time; error rate | Medium | Run the fresh-storage onboarding E2E path and record the first abandonment or ambiguity point. |
| 4 | A first meaningful action is measurable without collecting raw searches or personal details. | **Partial:** `lib/posthog.ts`, `lib/consent.ts`, `app/components/PageTracker.tsx`, and capture call sites exist; funnel completeness and property hygiene still require an event-by-event audit. | Product engineering | Consent-to-first-value funnel coverage; zero raw search terms or email properties | Medium | Inventory current event names and properties against page view, onboarding start/complete, discovery result view, and first shelf action; add only missing funnel events. |
| 5 | Errors are visible to the team without turning user sessions into surveillance. | **Partial:** Sentry instrumentation and `app/global-error.tsx` exist; delivery of a sanitized test error and alert thresholds are not evidenced in this repo. | Product engineering | Unhandled-error rate; affected sessions; alert acknowledgement time | Small | Verify one sanitized test error reaches Sentry and document the alert threshold; do not include fragrance notes, searches, or user-entered text. |
| 6 | The launch message sounds like a chemist's bench: observation, material, ritual, and personal memory—not a beauty marketplace or generic SaaS. | `DESIGN.md`, `NOTA-BRAND-UIUX-PACK.md`, `NOTA_LORE.md`, live homepage copy | Christopher | Message comprehension in five short user conversations; recurring misunderstood phrase count | Small | Test one sentence: what nota. notices, what the visitor does, and what they leave with; keep only language users can repeat back. |
| 7 | The most useful public route has a complete search preview. | `app/layout.tsx`, route metadata, OG routes under `app/api/og/` | Product engineering | Valid title/description/canonical/OG image for homepage and chosen entry route | Small | Validate rendered metadata and social cards for `/` and the single launch entry route. |
| 8 | Launch traffic has a controlled observation window and a rollback trigger. | **Blocked:** smoke scripts and monitoring integrations exist, but the 24-hour dashboard view and numeric stop conditions are not recorded here. | Christopher (go/no-go) + product engineering (monitoring) | 5xx rate, onboarding errors, completion rate, rollback trigger breached/not breached | Small | Write the 24-hour dashboard view and numeric stop conditions before announcing the URL. |
| 9 | Feedback becomes evidence, not an unbounded feature list. | `app/admin/feedback/page.tsx`, `docs/todo/README.md` | Christopher | Feedback items grouped by blocked journey step; decision made within seven days | Small | Triage each item into fix now, test next, watch, or reject, linked to the funnel step it affects. |
| 10 | Post-launch learning changes one priority at a time. | PostHog funnel, Sentry issue trend, `docs/todo/README.md` | Christopher | One weekly decision with outcome, evidence, impact, effort, risk, and next action | Small | Hold a 30-minute T+7 review; choose the highest-impact measured break in the launch journey and record one next decision. |

Launch is ready when items 1–8 have current evidence and owners have accepted their next action. Items 9–10 define the learning loop after the URL is announced.
