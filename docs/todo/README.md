...
## Added 2026-07-27 — from cross-repo cleanup audit + skill-tampering incident (see docs/lessons.md L36–L38)
...
- **Sweep other repos' `.claude/skills/` directories for name collisions with global skills**, same class of issue as the `repo-tidy` shadow found in scentral-hub (see `docs/lessons.md` L37). abundance, ai-ops, and last30days-skill weren't checked for this.
...
## Added 2026-07-27 — from the live table-usage audit (see `db-table-usage-audit` skill, `docs/lessons.md` L42)

- **Decide what to do about `trend_signals`**: 5 live rows of real trend-monitoring data (YouTube/Reddit fragrance mentions with a `pending`/`published` status field); no usage was found in the audited repository surfaces (`app/`, `lib/`, `components/`, `scripts/`, `supabase/functions/`, migrations for triggers/functions, and `.rpc(` calls — see L42's method) that reads it or transitions its status. Either an external pipeline you know about is meant to feed the app eventually, or this is an orphaned import — needs your answer, not another guess. An external consumer outside this repo (e.g. a separate pipeline/cron job/other codebase) can't be ruled out by a repo-only audit.
