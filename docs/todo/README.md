...
- **Six tables are genuinely dead weight** (verified, not just grepped — see L42's three-point method): `houses`, `swap_offers` (feature-wise; only referenced in a GDPR delete-list), `layering_patterns`, `layering_protocols`, `product_signals`, `fragrance_facts`. Confirm with the founder whether any were meant to ship and got dropped, or whether they're safe to drop from schema.
