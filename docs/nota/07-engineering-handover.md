...
- **`/api/pros-cons`** no longer crashes when Claude wraps JSON in a ```` ```json ```` fence — response is stripped before parsing, and parse/call failures return `{ success: false, unavailable: true }` (200) instead of a 500. Its consumer now renders a quiet "isn't available right now" line on failure instead of silently vanishing. (`/api/proscons` and `components/collection/ProsCons.tsx` — an earlier duplicate variant — have since been deleted; only `/api/pros-cons` survives.)
