...
1. **Pull the live schema, not the doc's claim.** Use `list_tables` (Supabase MCP) with `verbose: true` for column-level detail, or, if MCP access isn't available:
   ...
   **`information_schema.tables` is privilege-scoped, not a full catalog.** Postgres only returns rows the current role has SELECT or ownership visibility for — a role without full table visibility can silently see fewer tables than actually exist. Before trusting the row count as exhaustive, use the privileged Supabase MCP `list_tables` path, confirm the querying role is the relevant schema owner/superuser, or explicitly verify visibility for every relevant public table from an independently derived expected list. `USAGE` on `public` plus spot checks is not sufficient. If complete coverage cannot be proven, report the audit as **incomplete** for schema coverage and never classify a missing fallback row as ORPHANED.
...
3. **Before asserting ORPHANED, close the dynamic-access blind spot across ALL of the surfaces in step 2, not just `app/`** (see `docs/lessons.md` L69):
   - Inventory all migrations, not only the table's defining migration, for later `CREATE FUNCTION` / `CREATE TRIGGER` definitions that reference the candidate table.
   - Grep every surface from step 2 — `app/`, `lib/`, `components/`, `scripts/`, `supabase/functions/` — for `.rpc(` calls, then map each RPC call/definition path to the specific candidate table. An unrelated RPC is not evidence for or against the table.
   ...
   - If the RPC/function/trigger relationship cannot be mapped to the candidate table, classify that relationship as **UNKNOWN** and do not assert ORPHANED.
   - A table found in an unexpected place (e.g. a GDPR delete-list rather than a feature route) is a signal that other unexpected access patterns are plausible in this codebase — don't assume grep-zero across every surface means truly unused until all of the above checks also come back empty or mapped away on every surface.
4. **Classify each table:**
   ...
   - **ORPHANED** — no code reference, no migration function/trigger mapped to the table, no RPC call path mapped to the table, and no unresolved UNKNOWN relationship. State this only after step 3 is complete.
