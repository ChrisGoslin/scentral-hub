...
- **Rejected/decoy fix:** Installing the hook at `.git/hooks/pre-push` — this repo's local git config sets `core.hooksPath=.husky`, so `.git/hooks/` is never consulted. Confirmed the hard way (a first install there silently never fired). `core.hooksPath` is local-only and never committed, so every fresh clone must set it: `git config core.hooksPath .husky` (AGENTS.md "Local Dev Setup"). The hook file itself **is** committed at `.husky/pre-push` — corrected 2026-08-24, when the old copy/chmod setup was removed: it installed from `scripts/hooks/pre-push`, a source predating every guard, so following it silently disabled all of them (see docs/lessons.md L82).
...
- **Symptom:** Different docs in the same repo asserted wildly different fragrance counts and product-era facts as if they were current: `docs/PRODUCT_TRUTH.md` still says **282 fragrances** and the brand name "nota."; `docs/architecture.md`-era docs described a 76-fragrance catalogue, a 3-tab nav, and magic-link auth — "wrong on all 3" per `docs/PRODUCT_TRUTH.md:69`.
...
- **The actual fix, not a doc edit:** never cite a fragrance count (or any other decaying fact) from a doc at all — run `SELECT count(*) FROM fragrances` (via Supabase MCP `execute_sql` or `mcp__*__execute_sql`) and cite that. `grounded-agent-guardrails` §"Ground Truth (nota.)" table exists specifically so future agents stop hardcoding numbers that go stale.
