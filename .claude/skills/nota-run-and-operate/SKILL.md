...
| Sentry | Error tracking | `next.config.ts` lines 76/78: org `basenote-qn`, project `sentry-aquamarine-village` (verified by grep — note the org/project slugs still carry the pre-rebrand "basenote" name; this is a slug, not a display-layer rebrand target). **UNVERIFIED-in-prod**: whether errors are actually flowing into this org/project today — check the Sentry dashboard directly, do not assume config presence means it's working |
