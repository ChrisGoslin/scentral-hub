...
| `branch-hygiene` | Session-start checklist: sync state, check what already exists before building, branch-vs-main decision, safe commit at the end. | Partially — branch/main decision logic is stable; the "what already exists" check is always live (`find app -name page.tsx`, not hardcoded). |
| `safe-commit-shared-repo` | Commit safely when a concurrent session may be editing this repo at the same time. Explicit-pathspec discipline. | Yes — add new "what NOT to do" examples as you hit them. |
