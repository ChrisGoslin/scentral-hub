...
- **Phase 12 (2026-07-18):** PR #55/#64 CI response — fixed 5 Codex-flagged issues on PR #55 (product_signals RLS size caps, false-500 on insert after INSERT-only RLS, weekly-brief prompt char budget, oversized-source fail-fast in fragrance ingest, broken `undefined`-body migration) and mirrored the phantom-column backfill (`top_notes`/`heart_notes`/`base_notes`, `full_name`/`inspired_by`) onto PR #64's copy. **Known platform limitation (confirmed via Postgres logs, not guessed):** Supabase Preview branch creation for this project restores from a cached snapshot that already marks certain migration versions (`20260507000003`, and even a brand-new never-before-seen `20260507000005`) as "applied" without ever executing their SQL bodies — verified by the total absence of their `ALTER TABLE` statements in `pg_log` across two fresh branch replays, despite `schema_migrations` listing them as applied and the exact same SQL running cleanly when executed directly against the branch. This is a preview-branch-only artifact, not a code defect: `supabase db push` against a real target runs each file's current SQL directly, it doesn't inherit a stale snapshot's version-applied bookkeeping. Founder decision (2026-07-18): stop chasing this specific check via close/reopen cycles; leave it red as a documented, understood gap. All other checks (build/lint/typecheck/SonarQube/security) are green on both PRs.
