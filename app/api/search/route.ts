...
type FragranceRecord = Record<string, unknown> & {
  id: string
...
type SimilarityResult = {
  seed: FragranceRecord
  similarNotes: SimilarityRow[]
}
...
const searchLimiter = makeLimiter('search', 60, '1 m')

export async function GET(request: NextRequest) {
  const allowed = await enforce(searchLimiter, clientIp(request))
  if (!allowed) {
  ...
  try {
    ...
    if (mode === 'all' || mode === 'exact' || mode === 'smells_like') {
      ...
      exactMatches = (exactRes.data ?? []) as FragranceRecord[]
      inspiredMatches = (inspiredRes.data ?? []) as FragranceRecord[]
    ...
    if ((mode === 'all' || mode === 'smells_like') && exactMatches.length > 0) {
      ...
      const seeds = exactMatches
        .filter((f) => f.top_notes?.length || f.heart_notes?.length || f.base_notes?.length)
        .slice(0, MAX_SIMILARITY_SEEDS)

      // OPTIMIZED: Parallel RPC calls instead of sequential for loop
      if (seeds.length > 0) {
        const similarityPromises = seeds.map(async (seed): Promise<SimilarityResult> => {
          ...
          const { data, error } = await supabase.rpc('search_by_note_similarity', {
          ...
          return { seed, similarNotes: (data ?? []) as SimilarityRow[] }
        })
        ...
        similarityResults.forEach((result) => {
          result.similarNotes.forEach((row) => allIds.add(row.id))
        })
        ...
        if (allIds.size > 0) {
          ...
          const byId = new Map((fullRecords ?? []).map((f) => {
            const fragrance = f as FragranceRecord
            return [fragrance.id, fragrance] as const
          }))
          ...
          similarityResults.forEach((result) => {
            results.push(
              ...result.similarNotes
                .flatMap((row) => {
                  const fragrance = byId.get(row.id)
                  if (!fragrance) return []
                  return [{
                  fragrance,
